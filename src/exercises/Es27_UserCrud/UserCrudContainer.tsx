...
import { userServiceDay27 } from '../../services/userServiceDay27';
...
export default function UserCrudContainer() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      const result = await userServiceDay27.fetchUsers(currentPage, LIMIT, filter, simulateError);
  ...
  const fetchSingleUser = async (id: number | string) => {
    ...
    setErrorMessage(null);
    try {
      const user = await userServiceDay27.fetchUserById(id, simulateError);
  ...
  const handleDeleteUser = async (id: number | string) => {
    ...
    setErrorMessage(null);
    try {
      await userServiceDay27.deleteUser(id, simulateError);
      setView('list');
